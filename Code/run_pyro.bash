...
if [ "$file" = "" ]; then
	if [ "$norun" = "1" ]; then
		if [ "$runcluster" = "1" ]; then
			snakemake -j $threads -npr --quiet --cluster-config $clusterconfig --cluster "$clustercommand" --cluster-status scripts/slurm-status.py --jobs 100 $rule $unlock
		else
			snakemake -j $threads -npr --quiet $rule $unlock
		fi
	else 
		if [ "$runcluster" = "1" ]; then
			snakemake -j $threads -p --cluster-config $clusterconfig --cluster "$clustercommand" --cluster-status scripts/slurm-status.py --jobs 100 $rule $unlock
		else
			snakemake -j $threads -p $rule $unlock
		fi
	fi
else
	if [ "$norun" = "1" ]; then
                if [ "$runcluster" = "1" ]; then
                        snakemake -j $threads -npr --quiet --cluster-config $clusterconfig --cluster "$clustercommand" --cluster-status scripts/slurm-status.py --jobs 100 $file $unlock
                else
                        snakemake -j $threads -npr --quiet $file $unlock
                fi
        else
                if [ "$runcluster" = "1" ]; then
                        snakemake -j $threads -p --cluster-config $clusterconfig --cluster "$clustercommand" --cluster-status scripts/slurm-status.py --jobs 100 $file $unlock
                else
                        snakemake -j $threads -p $file $unlock
                fi
        fi
fi
